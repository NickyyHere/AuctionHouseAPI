...
namespace AuctionHouseAPI.Application.CQRS.Validators
{
    public class CreateAuctionOptionsValidator : AbstractValidator<CreateAuctionCommand>
    {
        public CreateAuctionOptionsValidator()
        {
            ...
            RuleFor(x => x.CreateAuctionDTO.Options.StartDateTime).GreaterThanOrEqualTo(DateTime.UtcNow);
            RuleFor(x => x.CreateAuctionDTO.Options.FinishDateTime).NotEmpty().GreaterThan(x => x.CreateAuctionDTO.Options.StartDateTime.HasValue ? x.CreateAuctionDTO.Options.StartDateTime.Value.AddDays(1) : DateTime.UtcNow.AddDays(1));
