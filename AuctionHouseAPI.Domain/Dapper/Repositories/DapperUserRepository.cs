...
namespace AuctionHouseAPI.Domain.Dapper.Repositories
{
    public class DapperUserRepository : DapperBaseRepository<User>, IUserRepository
    {
        ...
        public override async Task<int> CreateAsync(User entity)
        {
            ...
            var sql = "INSERT INTO \"Users\" (\"Username\", \"Email\", \"Password\", \"FirstName\", \"LastName\") VALUES (@Username, @Email, @Password, @FirstName, @LastName) RETURNING \"Id\";";
            var userId = await _connection!.ExecuteScalarAsync<int>(sql, new { entity.Username, entity.Email, entity.Password, entity.FirstName, entity.LastName }, _currentTransaction);
